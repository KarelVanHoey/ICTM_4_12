...
while True:
    ...
    while aruco_friend == []: # loop is needed for if no aruco is found due to sudden movements.
        ...
        aruco_friend, _ = our_position_heading(grab_image_warped(M, maxWidth, maxHeight))
    while aruco_enemy == []:
        aruco_enemy, _ = their_position_heading(grab_image_warped(M,maxWidth,maxHeight))

    target, green_out, red_out, blue_out = next_target(aruco_friend, enemy_goal_centre, aruco_enemy[0], green_out, red_out, blue_out)
    ...
    for i in blue_in:
            cv2.circle(warped, i,radius=5,color=(255,0,0),thickness=-1) 
    for i in green_in:
            cv2.circle(warped, i,radius=5,color=(0,255,0),thickness=-1) 
    for i in red_in:
            cv2.circle(warped, i,radius=5,color=(0,0,255),thickness=-1) 
    ...
    cv2.drawContours(warped, [np.array(enemy_goal,dtype="int32")], -1, (0,0,255), 3) #Note: deze structuur is nodig om normale array te kunnen gebruiken
    ...
    cv2.circle(warped, np.array(aruco_enemy[0],dtype="int32"), radius=5,color=(255,255,127),thickness=-1)  
